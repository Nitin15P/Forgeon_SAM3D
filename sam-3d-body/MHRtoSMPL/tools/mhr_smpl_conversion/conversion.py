...
class Conversion:
    ...
    def convert_sam3d_output_to_smpl(
        ...
        # These are the keys that may exist in the sam3d_outputs.
        # If pred_vertices is present we will use it (and pred_cam_t) directly.
        # Otherwise we fall back to model parameters (mhr_model_params/shape_params/expr_params).
        ...
        concatenated_sam3d_outputs: dict[str, torch.Tensor] = {}
        for k in SAM3D_OUTPUT_VARIABLES:
            values = [sam3d_output[k] for sam3d_output in sam3d_outputs if k in sam3d_output]
            if not values:
                continue
            concatenated_sam3d_outputs[k] = self._to_tensor(np.stack(values, axis=0))
