...
class MHRHead(nn.Module):
    ...
    def __init__(
        ...
        if MOMENTUM_ENABLED:
            ...
            # Note: The model may have been saved with a newer PyTorch version
            try:
                ...
                if "UInt32Storage" in str(e) or "unknown torch global" in str(e):
                    # The TorchScript model was saved with PyTorch 2.2.0+ which has UInt32Storage
                    # PyTorch 2.1.2 doesn't have this, and we can't easily patch the C++ unpickler
                    raise RuntimeError(
                        f"\n{'='*70}\n"
                        f"MHR Model Loading Error: PyTorch Version Mismatch\n"
                        f"{'='*70}\n"
                        f"The MHR TorchScript model was saved with PyTorch 2.2.0+ (has UInt32Storage),\n"
                        f"but you are using PyTorch {torch.__version__} which doesn't support it.\n\n"
                        f"SOLUTIONS (choose one):\n\n"
                        f"1. UPGRADE PYTORCH (Recommended):\n"
                        f"   pip install torch>=2.2.0 torchvision>=0.17.0\n\n"
                        f"2. USE MHR PYTHON LIBRARY (Alternative):\n"
                        f"   pip install git+https://github.com/facebookresearch/MHR.git\n"
                        f"   export MOMENTUM_ENABLED=1\n"
                        f"   (This uses the Python implementation instead of TorchScript)\n\n"
                        f"Original error: {e}\n"
                        f"{'='*70}\n"
                    ) from e
                else:
