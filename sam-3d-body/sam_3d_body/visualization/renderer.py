...
# This must be set before any OpenGL/pyrender imports
# pyrender supports GLX (with X display), EGL (headless hardware), and OSMesa (software)
if "PYOPENGL_PLATFORM" not in os.environ:
    if os.environ.get("DISPLAY"):
        # If DISPLAY is set (Xvfb or real display), prefer GLX (standard for X Window System)
        os.environ["PYOPENGL_PLATFORM"] = "glx"
    else:
        # In pure headless (no DISPLAY), try EGL first (hardware accelerated)
        # OSMesa can be used manually if EGL fails: export PYOPENGL_PLATFORM=osmesa
...
# Import pyrender - this will initialize OpenGL based on PYOPENGL_PLATFORM
# If it fails, the error will be caught when Renderer is actually used
try:
    import pyrender
except (ImportError, OSError) as e:
    # Store the error to re-raise later when renderer is actually used
    _pyrender_import_error = e
    pyrender = None
else:
...
class Renderer:
    ...
    def __call__(
        ...
        # Check if pyrender was imported successfully
        if pyrender is None:
            raise ImportError(f"pyrender could not be imported: {_pyrender_import_error}")
        
        # Try to create renderer with fallback for headless environments
        try:
            ...
            # If EGL fails, suggest alternatives
            current_platform = os.environ.get("PYOPENGL_PLATFORM", "not set")
            error_msg = (
                f"Failed to create offscreen renderer with {current_platform}: {e}. "
                f"Try one of these solutions:\n"
                f"  1. Use Xvfb: xvfb-run -a python [your_script.py]\n"
                f"  2. Use OSMesa: export PYOPENGL_PLATFORM=osmesa (requires: sudo apt-get install libosmesa6-dev)\n"
                f"  3. Configure EGL properly for your GPU drivers"
            )
    ...
    def render_rgba(
        ...
        try:
            ...
            raise RuntimeError(f"Failed to create offscreen renderer: {e}") from e
        # material = pyrender.MetallicRoughnessMaterial(
        #     metallicFactor=0.0,
        #     alphaMode='OPAQUE',
        #     baseColorFactor=(*mesh_base_color, 1.0))
