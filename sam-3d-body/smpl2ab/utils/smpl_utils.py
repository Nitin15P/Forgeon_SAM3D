...
def _load_smpl_from_npz(npz_path):
    """Load SMPL model from chumpy-free .npz file. Returns Struct for smplx."""
    data = np.load(npz_path, allow_pickle=True)
    data_dict = {k: np.asarray(v) for k, v in data.items()}
    return Struct(**data_dict)


def _find_smpl_npz(gender, model_type='smpl'):
    """Find SMPL .npz file. Prefers npz over pkl (chumpy-free). Returns path or None."""
    if model_type != 'smpl':
        return None
    gender_upper = str(gender).upper()
    candidates = [
        os.path.join(cg.smpl_folder, 'smpl', f'SMPL_{gender_upper}.npz'),
        os.path.join(cg.smpl_folder, f'SMPL_{gender_upper}.npz'),
        os.path.join(os.path.dirname(cg.smpl_folder), f'SMPL_{gender_upper}.npz'),
    ]
    for p in candidates:
        if os.path.exists(p):
            return p
    # Fallback to NEUTRAL if gender-specific model not found (common when only npz available)
    if gender_upper != 'NEUTRAL':
        return _find_smpl_npz('neutral', model_type)
    return None
...
def SMPL(gender, num_betas=10, model_type='smpl'):
    """Load SMPL model. Prefers .npz (chumpy-free) over .pkl when available."""
    npz_path = _find_smpl_npz(gender, model_type)
    if npz_path is not None:
        data_struct = _load_smpl_from_npz(npz_path)
        smpl = smplx.create(
